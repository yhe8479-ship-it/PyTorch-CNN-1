...
从训练曲线里可以看到，两个模型在 20 个 epoch 的训练过程中都完成了比较稳定的学习：  
loss 整体下降，accuracy 整体上升，说明模型确实在不断从 FashionMNIST 数据中提取有效特征。

---

### 1. LeNet-5：训练过程比较稳定，但模型能力有限

LeNet-5 的训练曲线整体比较平稳。  
从 loss 曲线来看，前几个 epoch 下降比较明显，后面下降速度逐渐变慢，最后趋于稳定。这说明模型在训练初期很快学到了一些基础特征，比如衣服轮廓、边缘形状、鞋子和包的大致结构等。
从 accuracy 曲线来看，训练准确率和验证准确率都在逐步上升，说明模型没有出现明显训练失败的情况。最终 LeNet-5 在测试集上的准确率为 **85.87%**。

不过，LeNet-5 的准确率上限相对有限。  
原因可能是 FashionMNIST 比普通 MNIST 手写数字更复杂，不同类别之间相似度更高，例如：

- T-shirt 和 Shirt 容易混淆；
- Coat、Pullover、Dress 在局部形状上有相似之处；
- Sneaker、Sandal、Ankle boot 都属于鞋类，也可能在某些样本上比较接近。

LeNet-5 的网络结构比较浅，卷积层数量和通道数都不多，因此它更适合学习一些基础图像特征。对于 FashionMNIST 这种类别更复杂的数据集，它的表达能力可能还不够强。

---

### 2. AlexNet：准确率更高，但验证集波动更明显

AlexNet 的整体表现比 LeNet-5 更好。  
从最终结果来看，AlexNet 在测试集上的准确率为 **89.93%**，比 LeNet-5 的 **85.87%** 高了大约 **4.06 个百分点**。
这个提升说明更深的网络结构确实带来了更强的特征提取能力。  
相比 LeNet-5，AlexNet 使用了更多卷积层、更大的通道数，并且引入了 ReLU、MaxPool、Dropout 等结构，因此它能够学习到更加丰富的图像特征。

从曲线来看，AlexNet 的训练准确率上升更快，后期可以达到 **90% 以上**。这说明模型在训练集上有较强的拟合能力。

但同时也可以看到，AlexNet 的验证集 loss 后期有一定波动，并不是一直稳定下降。
说明模型在训练集上继续变好时，验证集表现并没有同步稳定提升。也就是说，模型可能开始学习到一些训练集中的细节特征，而这些特征不一定能很好地泛化到新数据上。

这让我更直观地感受到：  
**模型更深、更复杂，确实可能带来更高准确率，但也会增加过拟合风险😒**
