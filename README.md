...
**LeNet-5 的训练曲线整体比较平稳。**  
从 loss 曲线来看，前几个 epoch 下降比较明显，后面下降速度逐渐变慢，最后趋于稳定。这说明模型在前几个 epoch 中先学到了一些比较明显的特征，比如衣服轮廓、鞋子的边缘、包的整体形状等；但到了后期，模型继续提升就会变得更困难。  
...
这是因为：**相比 LeNet-5，AlexNet 使用了更多卷积层、更大的通道数，并且引入了 ReLU、MaxPool、Dropout 等结构，因此它能够学习到更加丰富的图像特征。**

**AlexNet 的训练准确率上升更快。**  
从曲线看，后期可以达到 **90% 以上**。这说明模型在训练集上有较强的拟合能力。

**AlexNet 的验证集 loss 后期有一定波动，并不是一直稳定下降。**  
