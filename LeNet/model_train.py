...
def train_val_data_process():
    ...
    # 每一张原始图片是 28×28 的灰度图，通道数为 1
    # ToTensor() 会把图片转换成张量，形状为 [1, 28, 28]
    # 如果你的 LeNet 里 f5 的输入是 400，这里应该保持 28×28，不要改成 224×224
    ...
    # 训练集 DataLoader
    # batch_size=128 表示每次从训练集中取 128 张图片组成一个 batch
    # shuffle=True 表示每个 Epoch 开始前打乱训练样本顺序，有利于训练
    # 一个 Epoch 中大约有 48000 / 128 = 375 个训练 batch
    ...
    # 验证集 DataLoader
    # batch_size=128 表示每次从验证集中取 128 张图片组成一个 batch
    # 验证阶段不更新参数，所以 shuffle=False 更常见，方便结果稳定复现
    # 一个 Epoch 中大约有 12000 / 128 = 94 个验证 batch
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # b_x 是一个 batch 的图片，形状通常是 [batch_size, 1, 28, 28]
        # b_y 是这个 batch 对应的真实标签，形状通常是 [batch_size]
        for step, (b_x, b_y) in enumerate(train_dataloader):
            # 当前 batch 的实际样本数
            # 大多数 batch 是 128，但最后一个 batch 可能小于 128
            # b_x.size() 是整个 batch 的形状,即[128, 1, 28, 28]
            # 而b_x.size(0)是这个 batch 里有多少个样本 即128，后面要乘的是这个
    ...
    # 选择最优参数
    # 保存最高准确率下的模型参数到一个文件里
    torch.save(best_model_wts,'E:/PythonProject/LeNet/best_model.pth')
